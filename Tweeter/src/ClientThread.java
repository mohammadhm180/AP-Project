...
public class ClientThread implements Runnable {
    ...
    public void like() throws SQLException, IOException, ClassNotFoundException {
        ...
        if (database.getTweet(tweetID) != null) {
            database.like(database.getTweet(tweetID));
        } else if (database.getRetweet(tweetID) != null) {
            database.like(database.getTweet(tweetID));
        } else if (database.getReply(tweetID) != null) {
            database.like(database.getTweet(tweetID));
        } else if (database.getQuote(tweetID) != null) {
            database.like(database.getTweet(tweetID));
        }
    ...
    public void unlike() throws SQLException, IOException, ClassNotFoundException {
        String tweetID = (String) OIS.readObject();
        if (database.getTweet(tweetID) != null) {
            database.unLike(database.getTweet(tweetID));
        } else if (database.getRetweet(tweetID) != null) {
            database.unLike(database.getTweet(tweetID));
        } else if (database.getReply(tweetID) != null) {
            database.unLike(database.getTweet(tweetID));
